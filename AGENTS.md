...
- **`hammerspoon/init.lua`** — besides per-app mode auto-switching and floating-window centering (`hotkeyCenter`), exposes `zoom(dir)`: synthesizes a real pinch-magnify gesture via `hs.eventtap.event.newGesture("beginMagnify"/"endMagnify", amount)` (the only way to inject a trackpad gesture — no public API exists). Gesture-blind apps (Chromium/Electron, listed in `GESTURE_BLIND`) fall back to ⌘=/⌘−. Karabiner's `Hyper+=`/`Hyper+-` call it via `hs -c "zoom('in'/'out')"`. Tune `ZOOM_STEP`.
